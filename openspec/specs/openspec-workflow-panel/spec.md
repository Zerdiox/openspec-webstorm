...
The panel SHALL offer Explore, Propose and Backlog review for the project; Apply, Verify and Archive
for each change; and Promote for each open follow-up. Backlog review and Promote SHALL be offered
only where the project has a follow-ups backlog.

#### Scenario: A change row
- **WHEN** a change is listed
- **THEN** Apply, Verify and Archive are offered for it
